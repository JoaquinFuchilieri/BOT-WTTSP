...
(function () {
    ...
    window.showAlert = function (opts) {
        return new Promise((resolve) => {
            ...
            const defaultTitle = type === 'success' ? 'Éxito' : (type === 'danger' ? 'Error' : (type === 'warning' ? 'Advertencia' : 'Notificación'));
            const {
                title = defaultTitle,
                message = '',
                confirmText = 'Aceptar'
            ...
            const confirmBtnClass = type === 'danger' 
                ...
                : (type === 'warning' 
                    ? 'btn-dialog-warning' 
                    : (type === 'success' ? 'btn-dialog-success' : 'btn-primary'));
    ...
    // Override native window.alert to intercept any native popups and render custom dialog
